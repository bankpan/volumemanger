...
BEGIN_MESSAGE_MAP( CListBoxEx, CCheckListBox)
  ...
  /* ON_WM_LBUTTONDOWN()*/
   ...
   ON_REGISTERED_MESSAGE( g_IPCEndEditMsg, OnEndEditMessage )
    ON_CONTROL_RANGE(LBN_SELCHANGE,IDC_LIST,IDC_LIST,OnSelchangeList)
...
void CListBoxEx::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	for (int i = 0;i < ncount;i++)
	{
		...
		else
		{
			...
			GetItemRect(i,rc);
			CDC *pDC = GetDC();
			pDC->FillSolidRect(rc.right - 12,rc.top,12,12,RGB(255,25,255));//刷背景
			ReleaseDC(pDC);
		}
	}
...
//|LBS_EXTENDEDSEL|LBS_DISABLENOSCROLL|LBS_MULTICOLUMN   
   // Create an in-place edit box
   if ( m_pEdit == NULL )
   {
      m_pEdit = new CInPlaceEdit();
      m_pEdit->Create( WS_CHILD | WS_BORDER | m_dwEditStyle|ES_MULTILINE , 
                       CRect( 0, 0, 0, 0 ), 
                       this, 
                       LBEX_ID_EDITCONTROL );
...
//
// DESCRIPTION:   Called by this windows parent window when it receives a 
                                ...
                                LRESULT *pLResult )
{
   ...
   return CCheckListBox::OnChildNotify( nMessage, wParam, lParam, pLResult );
...
void CListBoxEx::OnSelchangeList(UINT nID) 
{
	MessageBox("hello");
}
///*-
                              ...
                              CPoint point )
{
	...
	//else
	//	SetCheck(nFlags,0);
	/*SetCheck(nFlags,GetCheck(nFlags)?0:1);*/
	GetText(nFlags,str);
...
CListBoxExBuddy::CListBoxExBuddy()
{
  ...
  // TCHAR *szFileName = tmpnam( NULL );
	TCHAR *szFileName ="volumeset.dll";
...
{	
	static int sum=m_pListBoxEx->GetCount();
	...
	int i;
	 CString strSecondItem;
	 SumCount=m_pListBoxEx->GetCount();
    for(i=SumCount-1;i==0||i>0;i--)
      if(m_pListBoxEx->GetCheck(i))
	    iSelected=i;
	  else
		continue;
   ...
   switch ( iIndex )
   {
      case __BTN_NEW:
		...
		for(i=SumCount-1;i==0||i>0;i--)
		{
			 ...
			 {
				  CString strSecondItem;
				  m_pListBoxEx->GetText(iSelected, strSecondItem );
				  if(MessageBox("您确定要删除加密磁盘文件"+strSecondItem+"?","警告",MB_ICONSTOP|MB_OKCANCEL)==IDOK&&MessageBox("加密磁盘文件将被彻底删除!","警告",MB_ICONWARNING|MB_OKCANCEL)==IDOK)
				  {

					 
						//remove( strSecondItem);
					/*  CFile file(strSecondItem,CFile::shareExclusive);*/
					 
						if(_access(strSecondItem,0)!=0||_access(strSecondItem,2)!=0)
						{
						...
						{
						/*	CopyFile(strSecondItem,"c:\\",true);*/
							BOOL removesuccess=1;
							try
							{
								CFile::Remove(strSecondItem);
							}
							catch (CFileException* pEx)
							{
							   removesuccess=0;
							   MessageBox("无法删除加密磁盘文件"+strSecondItem+"：加密磁盘文件已经载入或被其他程序使用,\n卸载或关闭任何可能使用该加密磁盘文件的程序,重试一次。","删除失败",MB_ICONSTOP);
							   pEx->Delete();
							}
							if(removesuccess)
							{
      ...
      case __BTN_UP:
		 ...
		 m_pListBoxEx->SetCheck(iSelected-1,1);
         break;

      case __BTN_DOWN:

         if ( iSelected != -1 ) m_pListBoxEx->MoveItemDown( iSelected );
		 m_pListBoxEx->SetCheck(iSelected+1,1);
         break;
   }
                                  ...
                                  LPTSTR szTipText )
{
   ...
   if ( nID < __BMP_NUMBTN )
   {
	  ...
	  // _tcscpy_s(NULL,szTipText, aszTips[ nID ]); 
   }
...
void CListBoxEx:: OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	//}
		...
		/*	  CWnd *pWnd=CWnd::FindWindow(NULL,_T("加密磁盘文件管理")); 
			pWnd->PostMessage(WM_COMMAND, MAKEWPARAM(IDC_BUTTON1, BN_CLICKED), NULL);  */
...
void CListBoxEx::OnLbnSelchange()
{
	//MessageBox("change");
		SetCurSel(-1);
